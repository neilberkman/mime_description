defmodule MimeDescription.Generator do
  ...
  # The hash of the currently embedded data lives in a comment in the
  # generated file itself, so change detection works on a fresh checkout
  # (e.g. in CI) with no local cache state.
  defp read_data_hash do
    with {:ok, content} <- File.read(@output_file),
         [_, hash] <- Regex.run(~r/^# Data hash: ([0-9A-F]+)$/m, content) do
      hash
    else
      _ -> nil
    end
  ...
  defp generate_elixir_module(descriptions) do
    ...
    # Data hash: #{compute_data_hash(descriptions)}
